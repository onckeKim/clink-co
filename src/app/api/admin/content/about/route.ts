...
import { getAboutPageContent, updateAboutPageContent } from "@/lib/admin/content-store";
import { aboutPagePatchSchema } from "@/lib/validations/admin-content";
...
export async function GET() {
  ...
  return NextResponse.json({ about: getAboutPageContent() });
}
...
export async function PATCH(request: Request) {
  ...
  const before = getAboutPageContent();
  const body = await request.json().catch(() => null);
  const parsed = aboutPagePatchSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.issues[0]?.message ?? "Invalid content." }, { status: 400 });
  }

  const about = updateAboutPageContent(parsed.data);
