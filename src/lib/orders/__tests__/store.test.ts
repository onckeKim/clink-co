...
describe("order number generation", () => {
  it("generates an order number in PREFIX-YYMMDD-NNNN format", () => {
    const order = createOrder(baseOrderInput());
    expect(order.orderNumber).toMatch(/^CC-\d{6}-\d{4}$/);
  });

  it("embeds today's date in the order number", () => {
    const order = createOrder(baseOrderInput());
    const datePart = new Date().toISOString().slice(2, 10).replace(/-/g, "");
    expect(order.orderNumber).toContain(`CC-${datePart}-`);
  });

  it("increments the daily sequence for successive orders", () => {
    const first = createOrder(baseOrderInput());
    const second = createOrder(baseOrderInput());
  ...
  it("produces a unique, retrievable order number every time", () => {
    const order = createOrder(baseOrderInput());
  ...
  it("resets the daily counter when the date changes", () => {
    ...
    try {
      const dayOneOrder = createOrder(baseOrderInput());
...
describe("createOrder idempotency", () => {
  it("returns the existing order instead of creating a duplicate for the same idempotency key", () => {
    const key = crypto.randomUUID();
    const first = createOrder(baseOrderInput({ idempotencyKey: key }));
    const second = createOrder(baseOrderInput({ idempotencyKey: key }));
    expect(second.id).toBe(first.id);
    expect(second.orderNumber).toBe(first.orderNumber);
  });

  it("looks up an order by its idempotency key", () => {
    const key = crypto.randomUUID();
    const order = createOrder(baseOrderInput({ idempotencyKey: key }));
